...
class TestMemoryObjectStream:
    ...
    async def test_receive_then_send_nowait(self):
        async def receiver():
            received_objects.append(await receive.receive())
    ...
    async def test_send_then_receive_nowait(self):
        send, receive = create_memory_object_stream(0)
        async with create_task_group() as tg:
    ...
    async def test_send_nowait_then_receive_nowait(self):
    ...
    async def test_receive_send_closed_send_stream(self):
        send, receive = create_memory_object_stream()
        await send.aclose()
    ...
    async def test_receive_send_closed_receive_stream(self):
        send, receive = create_memory_object_stream()
    ...
    async def test_cancel_receive(self):
        send, receive = create_memory_object_stream()
        async with create_task_group() as tg:
    ...
    async def test_cancel_send(self):
        send, receive = create_memory_object_stream()
        async with create_task_group() as tg:
            await tg.spawn(send.send, 'hello')
            await wait_all_tasks_blocked()
    ...
    async def test_clone_closed(self):
    ...
    async def test_close_send_while_receiving(self):
        send, receive = create_memory_object_stream(1)
        with pytest.raises(EndOfStream):
            async with create_task_group() as tg:
                await tg.spawn(receive.receive)
    ...
    async def test_close_receive_while_sending(self):
        send, receive = create_memory_object_stream(0)
        with pytest.raises(BrokenResourceError):
            async with create_task_group() as tg:
                await tg.spawn(send.send, 'hello')
