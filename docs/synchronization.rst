...
.. note:: If you're coming from :mod:`asyncio`, :class:`Future` differs from
   ...
   * Checking for ``asyncio.Future.cancelled()`` or ``asyncio.Future.done()`` can be achieved by comparing :attr:`~Future.status`; it would be equal to :attr:`~Future.Status.CANCELLED` or not equal to :attr:`~Future.Status.PENDING`, respectively.
   * The equivalent of ``asyncio.Future.add_done_callback(callback)`` is to create a task which calls ``callback`` after awaiting :meth:`~Future.wait`.
